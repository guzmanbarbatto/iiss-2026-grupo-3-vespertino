...
| **HU-4** | Administrador del Sistema | contar con endpoints de la API para accionar switches, iniciar/detener el Controlador y validar la consistencia de las habitaciones | poder operar la calefacción de forma remota y garantizar que la configuración de las habitaciones no tenga errores ni IDs duplicados. |
| **HU-5** | Dueño de Casa | que todas las operaciones de la API requieran autenticación mediante un token (Bearer Token) | proteger el acceso al sistema para que solo aplicaciones o personas autorizadas puedan controlar la calefacción de mi hogar. |